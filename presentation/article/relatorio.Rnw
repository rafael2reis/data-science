...
\begin{abstract}
Este trabalho tem como objetivo realizar uma análise descritiva da Vegetação Secundária na Amazônia Legal, entre os anos de 2008, 2010 e 2012, utilizando dados oficiais do governo brasileiro.
\end{abstract}

\section{Introdução}

Nos últimos 20 anos, a extração de madeira e a expansão da agricultura removeram >328.000 $km^2$ da Amazônia Brasileira - uma área maior do que a Polônia. Apesar das taxas de desmatamento terem diminuído desde 2005, a Amazônia Brasileira continua a perder 7.000 $km^2$ por ano. Além disso, mudanças no Código Florestal Brasileiro podem expor um adicional de 220.000 $km^2$ de floresta a desmatamento legal. Em contraste à maior parte do sul da Amazônia, onde as áreas desmatadas geralmente são utilizadas como pasto por um longo período, na Amazônia oriental e central, é comum que as áreas desmatadas sejam abandonadas após 5 anos sendo utilizadas como pasto para gado. Na Amazônia Brasileira, a área de floresta secundária cresceu de 29.000 para 161.000 $km^2$ entre 1978 e 2002. Essa enorme expansão de floresta secundária se tornará invevitavelmente parte de um plano de preservação, particularmente dado que o seu crescimento a partir de pastos abandonados representa uma maneira significativa de compensar a perda de carbono pelo desflorestamento na Amazônia. \cite{bird_mov_2013}

\section{Data Sets}

As principais bases de dados utilizadas são provenientes dos projetos PRODES \cite{prodes_website} e TerraClass \cite{prodes_website}, ambos do Instituto Nacional de Pesquisas Espacias (INPE). Foram utilizados também a base \emph{Tree Cover Gain} do \emph{Global Forest Watch} (GFW), as delimitações de estados e municípios do Instituto Brasileiro de Geografia e Estatística (IBGE) \cite{ibge_website}, as malhas de rodovias federais do Deparamento Nacional de Infraestrutura de Transportes (DNIT) \cite{dnit_website} e as áreas de unidades de conservação do Ministério do Meio Ambiente \cite{mma_website}.

Com exceção dos dados do GFW, todas as bases são arquivos no formato \emph{shapefile}. Um \emph{shapefile} armazena geometria não-topológica e informações de atributo para as feições espaciais num \emph{data set}. A geometria de uma feição é guardada como um \emph{shape} contendo um conjunto de vetor de coordenadas \cite{shapefile}.

\subsection{PRODES}

O Projeto de Monitoramento do Desmatamento na Amazônia Legal por Satélite (PRODES) é um projeto do INPE que utiliza sensoriamento remoto para identificar desmatamentos na Amazônia Legal. O projeto realiza o inventário de perda de floresta primária através do mapeamento da dinâmica do desmatamento por corte raso, que é aquele que resulta na remoção completa da cobertura florestal em um curto intervalo de tempo.
...
Após esse processamento, um outro \emph{script} realiza o cálculo das áreas de vegetação secundária e armazena os resultados num arquivo txt por estado/ano.

\subsection{Reprodutibilidade}

Um dos objetivos desta pesquisa era que ela pudesse ser reprodutível. Para isso, foi utilizado o pacote \emph{knitr} do R, que permite a inserção dos \emph{scripts} acima definidos no código \LaTeX{}.

Como os \emph{scripts} de preparação dos dados podem demorar até dias para terminar, foram disponibilizados os arquivos txt com os resultados do processamento. Além disso, foi inserida uma verificação no código, que dispara a preparação dos dados, caso os resultados não sejam encontrados.

Todos os scripts em R, bem como a documentação em \emph{knitr} foi disponibilizada no site de controle de versão Github em \cite{github_sv}. Os arquivos com os \emph{shapefiles} processados, assim como os fontes dos mapas em QGIS, podem ser baixados em \cite{shapefiles_drive}.

Como os arquivos QGIS fazem referência dinâmica aos \emph{shapefiles} fonte, eles podem ser reconstruídos a partir de todo o processamento. Porém, a etapa de separação, corte e união dos arquivos do TerraClass 2008 por estado devem ser feitas através do software. Futuramente, a fim de tornar a pesquisa completamente reproduzível, devem ser criados scripts em Python que executem esta tarefa.

<<dataprep,tidy=F,cache=F,eval=T,echo=F,results=hide>>= opts_chunk$set(autodep=T)
#print(getwd())
#setwd('/Users/rafael2reis/workspace/r/VegetacaoSecundaria/')

arquivosTerraClass <- list.files(path = "data/output/terraclass", pattern = paste0(".+\\.txt$"))
arquivosProdes <- list.files(path = "data/output/prodes", pattern = paste0(".+\\.txt$"))
arquivosIbge <- list.files(path = "data/output/ibge", pattern = paste0(".+\\.txt$"))

if (length(arquivosTerraClass) == 0 
    | length(arquivosProdes) == 0
    | length(arquivosIbge) == 0) {
  print('Ops!!!')
}
@
...
Inicialmente, com os dados de 2012, sobrepomos o desmatamento com as regiões de vegetação secundária, junto com as delimitações de municípios, na figura \ref{fig:sv_deforestation_2012}. Observamos que, mesmo quando consideramos os dados de apenas um ano (2012), para uma escala que engloba toda a Amazõnia Legal, há muita dificuldade em fazer qualquer tipo de análise. Tentamos reduzir a escala para o nível do estado do Pará, conforme mostrado na figura \ref{fig:sv_deforestation_PA_2012}. Percebemos que as regiões de vegetação começam a ficar mais evidentes, mas ainda existe certa dificuldade de observação. Isso acontece porque, em geral, os polígonos de desmatamento e vegetação são muito pequenos, que acabam diluídos ou desaparecendo quando observados desta distância.

Antes de entrarmos num nível maior de detalhe, vamos agregar algumas informações que poderão ser úteis para a continuação da análise.
...
Como a vegetação secundária surge a partir de áreas já desmatadas, para ilustrar, analisamos o desmatamento entre os anos 2002 e 2012 na figura \ref{fig:deforestation_2012}.
...
Estradas são importantes condutoras de desmatamento \cite{nelson1997roads} \cite{geist2002proximate}. Agregando esta informação na análise, inserimos as rodovias federais no mapa \ref{fig:deforestation_roads_2012}. Fica claro que a maior parte do desmatamento acompanha o curso das rodovias.
...
A seguir, investigamos se a vegetação secundária também aparece predominantemente no curso das estradas. Visualizando os dados de 2008, 2010 e 2012, obtemos o mapa da figura \ref{fig:sv_deforestation_2008_2010_2012}. Mesmo seguindo as recomendações para a coloração de mapas de \cite{colorbrewer}, como dito acima, essa escala dificulta a análise. Geramos o mapa com a expectativa de que seja útil para identificar regiões de investigações mais detalhadas.
...
Geramos, então, dois mapas, \ref{fig:sv_deforestation_AC_z2_2008_2010_2012} e \ref{fig:sv_deforestation_AC_zoom_2008_2010_2012}, com maior nível de detalhe para duas regiões diferentes do estado do Acre. Com a escala escolhida, podemos abservar comportamentos comuns aos dois mapas: confirmamos que a vegetação não segue o curso das rodovias e percebemos que, embora o surgimento de vegetação secundária seja pouco no ano de 2010, no ano de 2012 este surgimento é quase nulo.
